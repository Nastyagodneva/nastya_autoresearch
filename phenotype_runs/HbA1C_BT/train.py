...
def _raw_embed_select_k_best_prep(n_raw_features: int, k_raw: int = 300) -> ColumnTransformer:
    """Univariate raw feature selection (f_regression) + scale; embed branch scaled only."""
            ...
            (
                ...
                Pipeline(
                    [
                        ...
                        (
                            "skb",
                            SelectKBest(score_func=f_regression, k=min(k_raw, n_raw_features)),
                        ),
                        ("scaler", StandardScaler()),
                    ]
                ),
                slice(0, n_raw_features),
            ),
            (
                "embed",
                Pipeline(
...
def _raw_only_skb_prep(n_raw_features: int, k_raw: int = 300) -> ColumnTransformer:
    """Raw columns only: impute → SelectKBest → scale (embed columns dropped)."""
    return ColumnTransformer(
        transformers=[
            (
                "raw",
                Pipeline(
                    [
                        ...
                        (
                            "skb",
                            SelectKBest(
                                score_func=f_regression, k=min(k_raw, n_raw_features)
                            ),
                        ),
                        ("scaler", StandardScaler()),
                    ]
...
def _embed_only_scaled_prep(n_raw_features: int) -> ColumnTransformer:
    """Embed block only: impute → scale (raw columns dropped)."""
    return ColumnTransformer(
        transformers=[
            (
                "embed",
...
def _use_raw_embed_prep(feature_set, n_raw_features):
    return (
        feature_set == "raw+embed"
        and n_raw_features is not None
        and int(n_raw_features) > 0
    )
...
def make_model(
    ...
    use_re = _use_raw_embed_prep(feature_set, n_raw_features)

    if model_name == "elasticnet":
        if use_re:
            prep = _raw_embed_select_k_best_prep(int(n_raw_features))
            return Pipeline(
                [
                    ("prep", prep),
                    (
                            ...
                            [
                                ...
                                (
                                    ...
                                    SelectKBest(
                                        ...
                                        k=min(300, int(n_raw_features)),
                                    ),
                                ),
                            ]
                        ),
                        slice(0, int(n_raw_features)),
                    ),
                    (
                        "embed",
                        SimpleImputer(strategy="median"),
                        slice(int(n_raw_features), None),
                    ),
                ],
                remainder="drop",
                sparse_threshold=0.0,
        ...
        if use_re:
            n_raw = int(n_raw_features)
            prep_enet = _raw_embed_select_k_best_prep(n_raw)
            prep_ridge = _embed_only_scaled_prep(n_raw)
            prep_pls = _raw_only_skb_prep(n_raw)
            estimators = [
                (
                    "enet",
                    Pipeline(
                        [
                            ("prep", clone(prep_enet)),
                            (
                                "regressor",
                                ElasticNet(
                                    alpha=0.15,
        ...
        final_meta = Ridge(alpha=0.1, random_state=random_state)
